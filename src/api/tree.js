...
export default {
  ConvertJson2Tree (jsonArrObj) {
    ...
    console.log('jsonArrObj: ' + JSON.stringify(jsonArrObj))
    for (let i in jsonArrObj) {
      console.log('jsonObj: ' + JSON.stringify(jsonArrObj[i]))
      for (let key in jsonArrObj[i]) {
        console.log('key: ' + key)
        if (typeof (jsonArrObj[i][key]) === 'object') {
          this.ConvertJson2Tree(jsonArrObj[i][key])
          console.log('before push: ' + JSON.stringify(jsonArrObj[i][key]))
          console.log('push id : ' + jsonArrObj[i].id + '  parent: ' + jsonArrObj[i].id)
